...
macro_rules! define_Conf {
    ($(
        ...
        pub fn get_configuration_metadata() -> Vec<CrushyConfiguration> {
            vec![$(
                CrushyConfiguration {
...
define_Conf! {
    ...
    /// default configuration of Crushy. By default, any configuration will replace the default value.
    ...
    /// configuration of Crushy. By default, any configuration will replace the default value.
    ...
    /// `".."` can be used as part of the list to indicate that the configured values should be appended to the
    /// default configuration of Crushy. By default, any configuration will replace the default value. For example:
    ...
    /// The maximum byte size a `Future` can have, before it triggers the `crushy::large_futures` lint
    #[lints(large_futures)]
    ...
    /// When Crushy suggests using a slice pattern, this is the maximum number of elements allowed in
    ...
    /// The byte size a `T` in `Box<T>` can have, below which it triggers the `crushy::unnecessary_box` lint
...
pub fn lookup_conf_file() -> io::Result<(Option<PathBuf>, Vec<String>)> {
    ...
    const CONFIG_FILE_NAMES: [&str; 2] = [".crushy.toml", "crushy.toml"];

    // Start looking for a config file in CRUSHY_CONF_DIR, or failing that, CARGO_MANIFEST_DIR.
    // If neither of those exist, use ".". (Update documentation if this priority changes)
    let mut current = env::var_os("CRUSHY_CONF_DIR")
...
impl Conf {
    ...
    fn read_inner(sess: &Session, path: &io::Result<(Option<PathBuf>, Vec<String>)>) -> Conf {
        match path {
            ...
            Err(error) => {
                sess.dcx()
                    .err(format!("error finding Crushy's configuration file: {error}"));
            },
        }
        ...
        for error in errors {
            let mut diag = sess.dcx().struct_span_err(
                error.span,
                format!("error reading Crushy's configuration file: {}", error.message),
            );
        ...
        for warning in warnings {
            sess.dcx().span_warn(
                warning.span,
                format!("error reading Crushy's configuration file: {}", warning.message),
            );
        }
...
impl serde::de::Error for FieldError {
    ...
    fn unknown_field(field: &str, expected: &'static [&'static str]) -> Self {
        // List the available fields sorted and at least one per line, more if `CRUSHY_TERMINAL_WIDTH` is
        // set and allows it.
        use fmt::Write;
...
fn calculate_dimensions(fields: &[&str]) -> (usize, Vec<usize>) {
    let columns = env::var("CRUSHY_TERMINAL_WIDTH")
        .ok()
...
mod tests {
    ...
    fn configs_are_tested() {
        ...
        let toml_files = WalkDir::new("../tests")
            .into_iter()
            .map(Result::unwrap)
            .filter(|entry| entry.file_name() == "crushy.toml");
