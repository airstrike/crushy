...
/// * Renames all lint attributes to the new name (e.g. `#[allow(crushy::lint_name)]`).
...
pub fn rename<'cx, 'env: 'cx>(cx: ParseCx<'cx>, clippy_version: Version, old_name: &'env str, new_name: &'env str) {
    ...
    update_rename_targets(&mut data, old_name, LintName::new_crushy(new_name));
    ...
    let Lint::Active(mut prev_lint) = mem::replace(
        lint.get_mut(),
        Lint::Renamed(RenamedLint {
            new_name: LintName::new_crushy(new_name),
            version: cx.str_buf.alloc_display(cx.arena, clippy_version.rust_display()),
        }),
    ) else {
        eprintln!("error: `{old_name}` is already deprecated");
...
fn update_rename_targets<'cx>(data: &mut LintData<'cx>, old_name: &str, new_name: LintName<'cx>) {
    let old_name = LintName::new_crushy(old_name);
...
/// Creates an update function which replaces all instances of `crushy::old_name` with
/// `new_name`.
        ...
        loop {
            match cursor.peek() {
                ...
                TokenKind::Ident => {
                    ...
                    cursor.step();
                    match text {
                        // crushy::line_name or crushy::lint-name
                        "clippy" => {
