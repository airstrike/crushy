...
pub fn deprecate<'cx>(cx: ParseCx<'cx>, clippy_version: Version, name: &'cx str, reason: &'cx str) {
    ...
    let mod_path = {
        let mut mod_path = PathBuf::from(format!("crushy_lints/src/{}", lint.module));
        if mod_path.is_dir() {
        ...
        && let Some(lint) = lints.iter().find(|l| l.name == name)
    {
        if lint.module == name {
            ...
            if let Some(Some("mod.rs")) = path.file_name().map(OsStr::to_str) {
                // Remove crushy_lints/src/some_mod/some_lint.rs
            ...
            // Remove lint declaration (declare_crushy_lint!)
