...
pub fn create(
    ...
    if category == "cargo" && ty.is_none() {
        // `cargo` is a special category, these lints should always be in `crushy_lints/src/cargo`
        ty = Some("cargo");
    }
    ...
    if lint.ty.is_none() {
        add_lint(&lint, msrv).context("Unable to add lint to crushy_lints/src/lib.rs")?;
    }
...
fn create_lint(lint: &LintData<'_>, enable_msrv: bool) -> io::Result<()> {
    if let Some(ty) = lint.ty {
        ...
        let lint_path = format!("crushy_lints/src/{}.rs", lint.name);
...
fn add_lint(lint: &LintData<'_>, enable_msrv: bool) -> io::Result<()> {
    let path = "crushy_lints/src/lib.rs";
...
fn get_lint_file_contents(lint: &LintData<'_>, enable_msrv: bool) -> String {
    ...
    if enable_msrv {
        let _: fmt::Result = writedoc!(
            ...
            use crushy_utils::msrvs::{{self, {msrv_ty}}};
            {pass_import}
    ...
    if enable_msrv {
        let _: fmt::Result = writedoc!(
            ...
            // TODO: Add MSRV level to `crushy_utils/src/msrvs.rs` if needed.
            // TODO: Update msrv config comment in `crushy_config/src/conf.rs`
        "
        );
    } else {
        let _: fmt::Result = writedoc!(
...
fn create_lint_for_ty(lint: &LintData<'_>, enable_msrv: bool, ty: &str) -> io::Result<()> {
    ...
    let ty_dir = PathBuf::from(format!("crushy_lints/src/{ty}"));
    assert!(
        ...
        let _: fmt::Result = writedoc!(
            ...
            r#"
                ...
                pub(super) fn check(cx: &{context_import}{pass_lifetimes}, msrv: {msrv_ref}{msrv_ty}) {{
                    if !msrv.meets({msrv_cx}todo!("Add a new entry in `crushy_utils/src/msrvs`")) {{
                        return;
                    }}
                    todo!();
                }}
    ...
    println!("Generated lint file: `crushy_lints/src/{ty}/{}.rs`", lint.name);
    println!(
        "Be sure to add a call to `{}::check` in `crushy_lints/src/{ty}/mod.rs`!",
        lint.name
    );
...
// Find both the last lint declaration (declare_crushy_lint!) and the lint pass impl
fn parse_mod_file(path: &Path, contents: &str) -> (&'static str, usize) {
    ...
    while let Some(name) = cursor.find_any_ident() {
        match cursor.get_text(name) {
            "declare_crushy_lint" if cursor.match_all(&[Bang, OpenBrace], &mut []) && cursor.find_pat(CloseBrace) => {
