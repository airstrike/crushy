...
use crushy_dev::{
    CrushyInfo, UpdateMode, dogfood, edit_lints, fmt, lint, new_lint, new_parse_cx, release, serve, setup, sync,
};
use std::env;

fn main() {
    let dev = Dev::parse();
    let clippy = CrushyInfo::search_for_manifest();
...
enum DevCommand {
    ...
    /// * lint modules in `crushy_lints/*` are visible in `src/lib.rs` via `pub mod` {n}
    ...
    /// Launch a local 'ALL the Crushy Lints' website in a browser
    Serve {
    ...
    ///
    /// Set lint levels: {n}
    ///     cargo dev lint file.rs -- -W crushy::pedantic {n}
    ///     cargo dev lint ~/my-project -- -- -W crushy::pedantic
    Lint {
        ...
        path: String,
        /// Pass extra arguments to cargo/crushy-driver
        args: Vec<String>,
    },
    ...
    /// Sync between the rust repo and the Crushy repo
    Sync(SyncCommand),
    /// Manage Crushy releases
...
enum SetupSubcommand {
    ...
    /// `target/.../{crushy-driver,cargo-crushy}`, rebuilds of the project will be reflected in the
...
enum SyncSubcommand {
    ...
    /// Update nightly version in `rust-toolchain.toml` and `crushy_utils`
    UpdateNightly,
}
