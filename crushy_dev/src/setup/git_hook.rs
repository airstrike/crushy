...
/// the hook if `crushy_dev` would be used in the rust tree. The hook also references this tool
...
fn check_precondition(force_override: bool) -> bool {
    ...
    if !git_path.exists() || !git_path.is_dir() {
        eprintln!("error: crushy_dev was unable to find the `.git` directory");
        return false;
    }
