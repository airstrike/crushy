...
pub fn create(standalone: bool, force: bool, release: bool, name: &str) {
    ...
    for entry in WalkDir::new(&src) {
        ...
        if relative.starts_with("bin")
            && matches!(
                ...
                Some("cargo-crushy" | "crushy-driver")
            )
        {
            continue;
        }
    ...
    install_bin("cargo-crushy", &dest, standalone, release);
    install_bin("crushy-driver", &dest, standalone, release);

    println!("Created toolchain {name}, use it in other projects with e.g. `cargo +{name} clippy`");
    if !standalone {
        println!("Note: This will need to be re-run whenever the Crushy `rust-toolchain.toml` changes");
    }
}
