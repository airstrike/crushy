...
impl<'cx> LintName<'cx> {
    ...
    pub fn new_crushy(name: &'cx str) -> Self {
        Self {
            name,
            tool: LintTool::Crushy,
        }
    }
}
...
impl<'cx> ParseCxImpl<'cx> {
    ...
    pub fn parse_lint_decls(&mut self) -> LintData<'cx> {
        ...
        for e in expect_action(fs::read_dir("."), ErrAction::Read, ".") {
            ...
            let mut crate_path = if expect_action(e.file_type(), ErrAction::Read, ".").is_dir()
                ...
                && crate_path.starts_with("crushy_lints")
                && crate_path != "crushy_lints_internal"
            {
                crate_path
            } else {
                continue;
    ...
    /// Parse a source file looking for `declare_crushy_lint` macro invocations.
    fn parse_lint_src_file(&mut self, path: &Path, contents: &str, module: &'cx str, data: &mut LintData<'cx>) {
        ...
        static LINT_DECL_TOKENS: &[cursor::Pat<'_>] = &[
            ...
            Pound, OpenBracket, Ident("crushy"), DoubleColon, Ident("version"), Eq, LitStr, CloseBracket,
            // pub NAME, GROUP,
    ...
    fn read_deprecated_lints(&mut self, data: &mut LintData<'cx>) {
        ...
        let path = "crushy_lints/src/deprecated_lints.rs";
        let mut contents = String::new();
    ...
    fn parse_clippy_lint_name(&mut self, path: &Path, s: &str) -> &'cx str {
        match self.parse_str_single_line(path, s).strip_prefix("crushy::") {
            Some(x) => x,
            None => panic!(
                "error parsing `{}`: `{s}` should be a string starting with `crushy::`",
                path.display()
            ),
        }
    }
    ...
    fn parse_lint_name(&mut self, path: &Path, s: &str) -> LintName<'cx> {
        ...
        let (name, tool) = match s.strip_prefix("crushy::") {
            Some(s) => (s, LintTool::Crushy),
