...
/// `#[crushy::msrv]` attributes are rarely used outside of Crushy's test suite, as a basic
...
/// Tracks the current MSRV from `crushy.toml`, `Cargo.toml` or set via `#[crushy::msrv]` in late
...
impl Msrv {
    ...
    ///
    /// If the crate being linted uses an `#[crushy::msrv]` attribute this will search the parent
        ...
        match (self.0, cargo_msrv) {
            ...
            (Some(clippy_msrv), Some(cargo_msrv)) if clippy_msrv != cargo_msrv => {
                sess.dcx().warn(format!(
                    "the MSRV in `crushy.toml` and `Cargo.toml` differ; using `{clippy_msrv}` from `crushy.toml`"
                ));
            },
            _ => {},
        }
...
/// Tracks the current MSRV from `crushy.toml`, `Cargo.toml` or set via `#[crushy::msrv]` in early
...
fn parse_attrs(sess: &Session, attrs: &[impl AttributeExt]) -> Option<RustcVersion> {
    ...
    if let Some(duplicate) = msrv_attrs.next_back() {
        sess.dcx()
            .struct_span_err(duplicate.span(), "`crushy::msrv` is defined multiple times")
