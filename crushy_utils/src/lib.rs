...
pub fn fulfill_or_allowed(cx: &LateContext<'_>, lint: &'static Lint, ids: impl IntoIterator<Item = HirId>) -> bool {
    ...
    for id in ids {
        let spec = cx.tcx.lint_level_spec_at_node(lint, id);
        if let Some(expectation) = spec.lint_id() {
            cx.fulfill_expectation(expectation.into());
        }
...
pub fn is_lint_allowed(cx: &LateContext<'_>, lint: &'static Lint, id: HirId) -> bool {
    cx.tcx.lint_level_spec_at_node(lint, id).level() == Level::Allow
}
...
fn is_body_identity_function<'hir>(cx: &LateContext<'_>, func: &Body<'hir>) -> bool {
    ...
    //
    // Note: This is similar to `crushy_utils::get_last_chain_binding_hir_id`, but it works
