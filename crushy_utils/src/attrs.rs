//! Utility functions for attributes, including Crushy's built-in ones
...
/// Given `attrs`, extract all the instances of a built-in Crushy attribute called `name`
pub fn get_builtin_attr<'a, A: AttributeExt + 'a>(
    ...
    attrs.iter().filter(move |attr| {
            ...
            && *clippy == sym::clippy
        {
            let path_span = attr
                .path_span()
                .expect("Crushy attributes are unparsed and have a span");
...
/// If `attrs` contain exactly one instance of a built-in Crushy attribute called `name`,
...
/// Currently used to keep track of the current value of `#[crushy::cognitive_complexity(N)]`
